# Brief — UI-07: /artikel card category labels clip mid-word at 390px - the only mobile-only defect on the site

**Sprint:** 04 — *Fix what shipped* · **3 points** · owner `design-systems-engineer`
**Tracker:** `pnpm --silent sprint get UI-07 --sprint 4` (from `~/Documents/Code/buddy`)

**YOUR WORKTREE:** `~/orca/workspaces/hellokahwin-site/ui07-label-clip`
...
p.hk-eyebrow.truncate clientWidth >= its own scrollWidth at 390px on /artikel, proven by re-running docs/work-done/aug-30-2026-session-01/aug-31-2026-ui-04-EVIDENCE/harness/eyebrow3.mjs against production and getting 'actually clipped: 0' at 390, 768, 1024 and 1440. Today it is 171px box / 181px text = 10px hidden on 9 of 11 labels at 390 only. Fix must not shrink the label below 11px or hide it; the longest live labels (Pelamin Kad and Cenderahati Majlis, Sebelum Nikah: Jodoh Merisik and Tunang) must also fit.
...
**This is the ONLY mobile-only defect on the entire site.** UI-04 rendered every template at 390/768/1024/1440 and found exactly one thing that breaks on a phone and nowhere else. Everything else the owner saw on mobile is also visible on desktop.

So this item is small, bounded, and it closes the owner's original question. **Do not let it grow into a general mobile pass** — that pass has already been done.
