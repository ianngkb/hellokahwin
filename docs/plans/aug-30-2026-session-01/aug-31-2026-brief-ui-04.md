# Brief — UI-04: RENDERED audit of every template at real breakpoints, desktop AND mobile - runs FIRST

**Sprint:** 04 — *Fix what shipped - the front page is broken in production*
**Item:** `UI-04` · **8 points** · track `design`
**Owner:** `product-designer`
**Tracker:** `pnpm --silent sprint get UI-04 --sprint 4` (run from `~/Documents/Code/buddy`)

---

## Why this item exists — verbatim from the tracker

The CEO's 31 Aug audit could NOT render a mobile viewport: resize_window reported success while innerWidth AND outerWidth stayed 1920, a same-origin iframe was refused, and window.open was blocked. matchMedia (max-width:1023px) returned false throughout. So every rendered finding the CEO produced is desktop-at-1920 only, and the owner's report that MOBILE looks terrible is UNRESOLVED. Static CSS analysis says mobile is structurally sound - correct viewport meta, coherent 390/768/1024 breakpoints, one fixed width over 390px and it is on an internal admin page - which either means the complaint was about desktop, or there is a rendered defect that static analysis cannot see. Do not guess between those.

---

## Definition of done — verbatim from the tracker, and it is NOT negotiable

Every public template - homepage, article, category, /artikel index, /dewan-kahwin, search - captured at 390, 768, 1024 and 1440 CSS px on a REAL rendered viewport, with matchMedia asserted at each width so the breakpoint is PROVEN rather than assumed. One screenshot per template per breakpoint, committed. Every defect logged with the COMPUTED VALUE that evidences it - a width, a right-edge, a ratio - never a description alone. Explicitly answer: is there a mobile-only defect, yes or no, with evidence either way.

---

## Brief — verbatim from the tracker

Read docs/plans/aug-30-2026-session-01/aug-31-2026-audit-ui-desktop-mobile.md first - section 3 lists five candidate findings the CEO KILLED by measuring, and re-reporting any of them is a fail. Lazy-loaded images report naturalWidth 0 and are NOT broken. Empty alt on a card thumbnail inside a titled link is CORRECT. THIS ITEM IS DISCOVERY: it is expected to add items to the sprint, and finding nothing on mobile is a valid and valuable result.

---

## What the CEO wants you to know beyond the tracker

**YOU RUN FIRST, AND YOU ARE DISCOVERY.** Nothing waits on you, but everything is better if you land early — you are the only item that can settle whether a MOBILE defect exists at all.

**The CEO could not render a mobile viewport and did not pretend to.** Three attempts failed: `resize_window` reported success while `innerWidth` AND `outerWidth` stayed 1920; a same-origin iframe was refused; `window.open` was blocked. `matchMedia('(max-width:1023px)')` returned false throughout. **So the owner's report that mobile looks terrible is UNRESOLVED, and you are the item that resolves it.**

**Assert the breakpoint, do not assume it.** At each width, evaluate `matchMedia` and record the result alongside the screenshot. A review that believes it is at 390px while the viewport is 1920px produces confident, worthless findings — that is exactly what nearly happened here.

**Finding NO mobile defect is a valid and valuable result.** Say so plainly with evidence. Do not manufacture a finding to justify the item.

---

## Where this sits in the sprint

**Wave 1, runs first.** Discovery. The sprint is expected to GROW from what you find.
