# CONT-14 — Re-angle the hantaran-kahwin legacy seed toward definition and money

**Sprint 05 — *Build where the click is*** · track `content` · **3 points** · owner `writer-inspirasi-vendor-venue`
**Dispatched:** 01 September 2026
**Your item in the tracker:** `pnpm --silent sprint get CONT-14 --sprint 5` (run from `~/Documents/Code/buddy`)

## WHY THIS ITEM EXISTS

The one C2.1 boundary left OPEN in ceo-memory.md. Decision 120 assigned this to CONT-12 and CONT-12 closed without doing it - a sub-commitment that evaporated inside a closing item, which is the same pattern as RIGHTS-03 in this sprint. Changes one body, not the count.

## DEFINITION OF DONE — verbatim, and it is not narrowed

The live page's <h1>, <title> and opening 200 words centre on definition and money, quoted from live HTML before and after. No new URL is created and no redirect is added - the sitemap count is unchanged at whatever CONT-13/16 leave it, stated explicitly. The C2.1 boundary line in ceo-memory.md is closed with the measurement that closed it, or is left open with a stated reason.

## BRIEF

Read the C2.1 boundary section of docs/boardroom/ceo-memory.md first - the decision was deliberately left undecided pending a SERP check, so run that check before rewriting anything. /humanizer before done.
...
```
pnpm --silent sprint set-state CONT-14 in_progress --sprint 5
pnpm --silent sprint add-evidence CONT-14 --sprint 5 \
