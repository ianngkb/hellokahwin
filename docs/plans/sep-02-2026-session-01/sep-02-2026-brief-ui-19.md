# UI-19 — Finish the article rail: ship SUMBER under the CEO ruling, and stop the rail collapsing without it

**Sprint 06 — *Deepen where the click is*** · `design` · **8 points** · owner `design-systems-engineer`
**Dispatched:** 02 September 2026 · space: **the SITE space** · integration branch **`master`**
**Your row:** `pnpm --silent sprint get UI-19 --sprint 6` (from `~/Documents/Code/buddy`)
...
At 1024/1440/1920 on LIVE production, on at least three articles of different lengths: the rail renders to the RIGHT of the body (rail.left > body.right by computed getBoundingClientRect, not by markup presence), carrying REKOD, the contents list and - where sources exist - SUMBER, in that order. ON AN ARTICLE WITH NO SOURCES the rail still renders and does NOT collapse, and no empty SUMBER heading is printed: prove both with a computed-geometry capture on one article from each set. At 390px the same blocks render full-width in the same reading order. Body measure stays within UI-10's 45-75 characters per line at all three widths, figure stated at each. UI-06's layout gate carries a case that fails when the rail collapses AND a case that fails when an empty SUMBER heading is emitted, each with a committed fixture proven to fire on it and clear on an input differing in exactly that one property.
...
```
pnpm --silent sprint set-state UI-19 in_progress --sprint 6
pnpm --silent sprint add-evidence UI-19 --sprint 6 \
