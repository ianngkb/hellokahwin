# COPY-01 — The undatable 'akan datang tidak lama lagi' line is still live, and a shipped spec says it was already replaced

**Sprint 06 — *Deepen where the click is*** · `content` · **2 points** · owner `managing-editor`
**Dispatched:** 02 September 2026 · space: **the DOCS space** · integration branch **`feat/command-centre-dashboard`**
**Your row:** `pnpm --silent sprint get COPY-01 --sprint 6` (from `~/Documents/Code/buddy`)
...
DES-03 section 7.2 C records that its approved copy 'Kategori ini masih kosong.' plus a way out ALREADY replaced production's undatable 'akan datang tidak lama lagi' line. It did not - the line is live on 4 empty clusters after DES-08. DES-07 escalated exactly this question to the managing editor and it was never answered. Done when the empty-CLUSTER copy is decided and either shipped or explicitly kept with a reason. NOTE THE STATE DISTINCTION, it is the trap here: section 7.2 C's copy is for a FULLY EMPTY CATEGORY, not for an empty cluster inside a populated pillar, so it is not a drop-in substitute and copying it across would be solving the wrong state. Also correct DES-03 section 7.2 C, which asserts a replacement that never happened.
...
```
pnpm --silent sprint set-state COPY-01 in_progress --sprint 6
pnpm --silent sprint add-evidence COPY-01 --sprint 6 \
