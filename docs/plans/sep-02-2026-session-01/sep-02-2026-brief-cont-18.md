# CONT-18 — Build out nikah-undang-undang - lafaz taklik, rukun and syarat, all document intent and all underbuilt

**Sprint 06 — *Deepen where the click is*** · `content` · **8 points** · owner `writer-adat-agama-prosedur`
**Dispatched:** 02 September 2026 · space: **the DOCS space** · integration branch **`feat/command-centre-dashboard`**
**Your row:** `pnpm --silent sprint get CONT-18 --sprint 6` (from `~/Documents/Code/buddy`)
...
FOUR ARTICLES LIVE IN PRODUCTION, each 200 on first request with the status line quoted, taking the sitemap to 119 combined with CONT-17 (state the combined figure and who contributed what). Each contains the COMPLETE artefact - the full lafaz text, the complete list of rukun with each one named, the full set of syarat - quoted first and last line from live HTML. Every religious or legal claim carries its NAMED AUTHORITY AND EDITION and the date checked, per item. Where a state enakmen is quoted, the state and the enakmen are both named. PRE-FLIGHT #1 and #3 exit codes recorded per target.
...
```
pnpm --silent sprint set-state CONT-18 in_progress --sprint 6
pnpm --silent sprint add-evidence CONT-18 --sprint 6 \
