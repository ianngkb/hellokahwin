...
node harness/h1face.mjs                              # which face each template's h1 paints in
```
...
| `measurements/measure-preview-merged.json` | The preview after merging master's 18 commits                                                          |
| `measurements/measure-prod-after.json` | Live production after `aeb9f0e` deployed                                                                  |
| `measurements/measure-prod-final.json` | Live production after `73ab99b` deployed — **the DoD evidence**                                           |
| `measurements/gate-prod-before.json`   | `ui-layout-gate` against live production before the fix: `reading-measure 5`                              |
| `measurements/gate-preview-after.json` | The same gate against the merged preview: `reading-measure 0`                                             |
| `measurements/gate-prod-after.json`    | The same gate against live production after the fix: `reading-measure 0`                                  |
...
`harness/h1face.mjs` is the probe that settled it, by rendering each template's
own `h1` text to a canvas in its computed stack and comparing the advance against
each candidate face: homepage 848.2px and category 484.1px match Bodoni Moda,
while the **article** h1's 848.2px matches Georgia (870.0px as Bodoni — not it).
