...
| 01 | Aug 31 2026 | [RISK-09: the docs/site boundary guard built and proven — 27/27 in a throwaway clone, then INSTALLED in both live trees and re-proven against the installed hooks — refusal and a passing in-space control quoted from each repo; one install covers 15 site trees, and `pre-merge-commit` never fires here because a docs/site merge always conflicts, so `pre-commit` is the real interception point](aug-30-2026-session-01/aug-31-2026-done-risk-09-boundary-guard.md) | design-systems-engineer | completed |
...
| 01 | Sep 1 2026 | [RISK-10: the boundary guard is installable from a fresh clone — `scripts/git-hooks/` lived on the docs branch only, so the documented install command named a path that does not exist on the branch a clone lands on (0 files on `master`, 7 on the docs branch); the fixture question RISK-09 left open decided by classifying every ref by TREE CONTENT with `hk_space()` extracted verbatim by `sed` from the shipped hook, and a hard `exit 2` that runs nothing and says so rather than skipping its own check — run against a repo that genuinely cannot supply the fixture, not asserted; `install-hooks.sh` now defaulting to the repository it lives in, because the old default installed into somebody else's checkouts from a fresh clone and that loosening is written back into the dispatch pre-flight it weakens; case 0e, which caught the two copies of the guard drifting for real; and the item's own test reporting "master is missing the guard" when it was its clone that had failed on MAX_PATH](sep-01-2026-session-01/sep-01-2026-done-risk-10-hooks-on-master.md) | BMAD | completed |
| 01 | Sep 01 2026 | [SEO-13: 39 articles emitted no FAQPage and now 0 do — 74 present / 0 absent / 14 reasoned not-applicable on live production, 192 emitted questions to 327; the 39 decomposed 24 content : 14 not-applicable : **1 emitter**, so the fix the item's title invites was 1/39 of it; the brief's own 46/39/1-failed was one article short because a transient fetch failure had silently subtracted one from a coverage count; 120 Q&A pairs written from the articles' own bodies and guarded by a committed token gate with a negative control (a fabricated `RM77.50` Terengganu circular fails on six tokens) whose FIRST run failed 20 of 24 files with 16 of those being the gate itself, on Malay sentence openers — the dangerous direction for a new gate being NOISY rather than quiet, because a gate that cries wolf gets switched off; two articles nearly dropped from the count because two scripts each answered "does this already have a block?" with their own `/soalan lazim/i` over serialised JSON and both said yes to articles that merely CITE the JAIS soalan lazim page in prose, caught only because the dry run and the live census contradicted each other; the DoD reading that would have filed `borang-nikah`, `rukun-nikah` and `garden-wedding` as not-applicable and closed the item in an hour, rejected as a narrowing wearing literalism's clothes; and the stale `31` in ceo-memory.md replaced not by a fresh number but by a script that exits non-zero](sep-01-2026-session-01/sep-01-2026-done-seo-13-faq-schema-coverage.md) | head-of-seo-content | completed |
