 ...
 *   pnpm ui:gate:selftest                   # 431 assertions: fires AND clears (CI)
 *
...
async function collect(limits) {
  ...
  // ── CHECK 15: the rail is THERE, at a size, on the article template ───────
  //
  ...
  // ── CHECK 16: no empty `Sumber` heading ──────────────────────────────────
  //
  ...
  for (const img of imgs) {
    ...
    // ── CHECK 15 (UI-15): UI-03 R1 on the grid thumbnail ────────────────────
    //
    // `grid-thumb-variant` is CHECK 12 and it ran above the decode guards,
    // because it reads a filename. This one needs the geometry, so it runs on
    // the SAME `probe` and the SAME `r` the checks above used, deliberately: a
    // second measurement of the same element is a second chance to measure it
    // differently, and the intrinsic-size trap this file documents at length
    // was found exactly once and is not being reopened.
    //
    // ⚠ THE SCOPE TEST IS A CONDITION, NOT AN EARLY `continue`. It was a
    // `if (!img.matches(GRID_THUMB_SELECTOR)) continue;` while this check was
    // last in the loop, and UI-16's shaped-slot checks then merged in ABOVE it.
    // Had the `continue` survived the merge in its original position it would
    // have skipped R2 and R6 for every image that is not a grid thumbnail —
    // silently retiring two blocking checks on the article cover, the exact
    // slot UI-16 shipped them for, while every selftest that does not render an
    // article cover stayed green. Folded into the `if` so the loop body has one
    // exit and a later check cannot be orphaned by position.
    //
    ...
    if (img.matches(GRID_THUMB_SELECTOR) && dev > R1_MAX_ASPECT_DEVIATION) {
      violations.push({
...
const CHECKS = [
  'empty-content',
  'rail-collapsed',
  // UI-19, and they are checks 15 and 16 — this item built them as 13 and 14,
  // and UI-16 merged first with `shaped-slot-variant` and `shaped-slot-dims`
  // under those numbers. The NAMES never collided, which is why every log line,
  // every assertion and every fixture here keys on a name; the ordinals exist
  // only for the prose and are the part two branches can both claim.
  //
  // Two checks, not one, and neither is a rename of `rail-collapsed`.
...
async function selftest() {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 7c. CHECKS 15 & 16 — UI-19. `rail-missing` and `sumber-empty`.
  //
  ...
  for (const w of WIDTHS) {
    ...
    for (const c of ['rail-missing', 'sumber-empty'])
      assert(
        ...
        `pre-rail/article.html @${w}: ${c} CLEAN — the 01 Sep capture predates .hk-article-grid, so check 15 cannot speak about it and check 10 is the one that must`,
        listing(
          railBad.find((r) => r.name === 'pre-rail/article.html' && r.width === w),
          c,
        ),
...
if (has('selftest')) {
  ...
  // The two `-ok` files are controls for CHECKS 15 AND 16 — both must read
  ...
  console.error(
    'usage: node scripts/ui-layout-gate.mjs (--fixtures [--green] | --pre-rail | --rail | --discriminator | --h6-order | --grid-thumb | --empty-shell | --base <url> | --url <u>… | --selftest)\n' +
